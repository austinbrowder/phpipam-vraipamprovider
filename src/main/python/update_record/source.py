...
def do_update_record(self, auth_credentials, cert):
    ...
    for update_record in self.inputs["addressInfos"]:
        update_result.append(update(resource, self.inputs, update_record, ipam))
...
def update(resource, inputs, update_record, ipam):
    try:
        ip = update_record["address"]
        logging.info(f"Updating IP allocation {str(ip)}")
        addressObjects = ipam.get('/addresses/search/'+ip+'/')
        for addressObj in addressObjects:
          ipam.patch('/addresses/'+addressObj["id"]+'/', {
            "id": addressObj["id"],
