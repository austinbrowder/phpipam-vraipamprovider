...
def do_allocate_ip(self, auth_credentials, cert):
    ...
    try:
        ...
        for allocation in self.inputs["ipAllocations"]:
            allocation_result.append(allocate(resource, allocation, self.context, self.inputs["endpoint"], ipam))
...
def allocate(resource, allocation, context, endpoint, ipam):

    last_error = None
    for range_id in allocation["ipRangeIds"]:
        logging.info(f"Allocating from range {range_id}")
        try:
            return allocate_in_range(range_id, resource, allocation, context, endpoint, ipam)
        except Exception as e:
            last_error = e
...
def allocate_in_range(range_id, resource, allocation, context, endpoint, ipam):
    ...
    if int(allocation["size"]) == 1:
      ...
      raise Exception("Allocating block of IPs Not implemented.")
...
def rollback(allocation_result, ipam):
    for allocation in reversed(allocation_result):
        ...
        for allocatedIp in ipAddresses:
